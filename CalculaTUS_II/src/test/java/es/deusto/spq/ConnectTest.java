...
/*
 * ATENCIÓN!!!!!!!! ESTA CLASE SE USA CUNADO SE CAMBIAN LOS METODO DE LA CLASE
 * CONNECT A PUBLICOS SI NO NO SE PUEDE CUANDO SE CAMBIEN SE PUEDE QUITAR EL
 * COMENTARIO
 */

/*
 * class BdTest { //probaremos el funcionamiento de la base de datos usando las
 ...
 * usuario = new Users("Elena", "Alonso", "elena.alonso@deusto.es", "mesa555",
 * 1, "null"); c.RegisUser(usuario); Boolean
 * resultado=c.buscar_usuario("elena.alonso@deusto.es"); assertTrue(resultado);
 * //probamos que se realize correctamente la insercción de locales y la
 * busqueda de ellos Local l=new Local("bm", "santana", 48190, "nnana");
 * resultado=c.RegisLocal(usuario, l);//ya esta en la base de datos por lo que
 * deberia de ser false assertFalse(resultado);
 * resultado=c.buscar_local(usuario, l); assertTrue(resultado); //comprobamos
 * que funcioa la funcion para seber si es admin resultado=c.isAdmin(usuario);
 * assertTrue(resultado); //cambiamos el tipo de usario Users trabajador = new
 * Users("Erik", "Alonso", "erik.alonso@deusto.es", "mqwemef", 0,
